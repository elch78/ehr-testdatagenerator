...
| `Mother` | A resolved mother; `generate()` renders **only** what the mother sets — an unset field is omitted (even if mandatory) and `generate()` never throws. A `$random` directive (`{ "$random": {} }`) is resolved to a schema-typed random value. Generation follows `$ref` and recurses into nested objects and arrays. A `$type` directive (`"$type": "Patient"`) points generation at a named `definitions` entry instead of the schema root — needed when the root **or a nested field** is a `oneOf` over many types (FHIR: the root resource, and `Bundle.entry.resource`, where a composed `$mother` carries its own `$type` to disambiguate per entry); it is not a schema property, so it is never rendered. Object and array are detected **structurally**, not by a declared `type`: a schema with `properties` is an object, one with `items` is an array (FHIR's definitions omit `type`). |
...
- Schema support is `object` (incl. **nested** objects), **arrays** (of scalars or objects), scalar
  ...
  runs end to end — both a single resource (`FhirPatientExampleTest`) and a multi-resource **Bundle**
  whose entries compose other mothers and reference one another (`FhirBundleExampleTest`: a `collection`
  Bundle of a Patient and an Observation that points at it via `subject`). Property schemas without a
  `type` (`const`/`enum`/bare `$ref`, as FHIR uses) are treated as scalars.
